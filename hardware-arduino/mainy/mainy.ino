...
void setup() {
    ...
    */
    ...
    sendtopython("Arduino connected and says hi!");
}
...
void sendtocpp(char *data) {
    ...
    for (int i = 0; data[i] != '\0'; i++) {
        Serial.write(data[i]);  // Send each character as a byte
    } 
    Serial.write("\n");
    // Serial.println(data);
} // END sendtopython()
...
void buttontopython(char page [2], char button [2]) {
    ...
    // sprintf(cmd, "b%s%s%ld", page, button, state); OLD
    sprintf(cmd, "b%s%s%ld", page, button, state); // new for testing with c++
